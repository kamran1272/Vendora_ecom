...
import { getWarehouseProducts, type WarehouseProduct } from '../../services/product-warehouse.service'
...
export function SellerProductFormPage() {
  ...
  useEffect(() => {
    if (id) return
    void getWarehouseProducts({ page: 1, limit: 100 }).then((response) => setWarehouseProducts(response.items)).catch(() => setError('Unable to load warehouse products.'))
  }, [id])
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!id && !warehouseProductId) { setError('Select a warehouse product before creating a listing.'); return }
    ...
    try {
      ...
      else await createSellerProduct({ ...payload, warehouseProductId })
  ...
  return <SellerLayout title={id ? 'Edit product' : 'Create product'} subtitle={id ? 'Update the seller price and visibility for this warehouse-backed product.' : 'Create a seller listing from an approved warehouse product.'} actions={<button type="button" onClick={() => navigate('/seller/products')} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700"><ArrowLeft size={15} /> Back to products</button>}>
    {loading ? <div className="animate-pulse rounded-2xl border border-slate-200 bg-white p-10 text-slate-500">Loading product...</div> : <form onSubmit={submit} className="space-y-6">
      ...
      {!id && <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-bold text-slate-900">Warehouse product</h2><p className="mt-1 text-xs text-slate-500">Choose an approved warehouse product. Its master details remain managed by Vendora.</p><select value={warehouseProductId} onChange={(event) => { const selected = warehouseProducts.find((product) => product.id === event.target.value); setWarehouseProductId(event.target.value); if (selected) setForm((current) => ({ ...current, name: selected.name, slug: selected.slug || slugify(selected.name), category: selected.category || '', brand: selected.brand || '', sku: selected.sku, price: String(selected.basePrice), description: selected.description || '' })) }} className={`${inputClass} mt-4`} required><option value="">Select a warehouse product</option>{warehouseProducts.map((product) => <option key={product.id} value={product.id}>{product.name} ({product.sku})</option>)}</select></section>}
