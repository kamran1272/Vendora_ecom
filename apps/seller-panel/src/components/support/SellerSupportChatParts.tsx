...
export function SellerChatHeader({ online, unreadCount, onMinimize, onClose }: { online?: boolean; unreadCount: number; onMinimize: () => void; onClose: () => void }) {
  return <header className="flex items-center justify-between bg-[#2d80d8] px-4 py-3 text-white"><div className="flex min-w-0 items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white p-1"><img src="/Vendora_Logo_Icon.png" alt="" className="h-full w-full object-contain" /></div><div className="min-w-0"><h2 className="truncate text-sm font-bold">Vendora Support</h2><p className="flex items-center gap-1 text-xs text-blue-100"><span className={`h-1.5 w-1.5 rounded-full ${online ? 'bg-emerald-300' : 'bg-slate-300'}`} />Customer Support · {online ? 'Online' : 'Offline'}</p></div></div><div className="flex items-center gap-1"><span className="sr-only">{unreadCount ? `${unreadCount} unread messages` : 'No unread messages'}</span><button type="button" aria-label="Minimize Vendora Support" onClick={onMinimize} className="rounded-lg p-2 text-blue-100 transition hover:bg-white/15 hover:text-white"><Minimize2 size={17} /></button><button type="button" aria-label="Close Vendora Support" onClick={onClose} className="rounded-lg p-2 text-blue-100 transition hover:bg-white/15 hover:text-white"><X size={18} /></button></div></header>
}
